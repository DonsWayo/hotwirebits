...
export default class extends Controller {
  ...
  render() {
    this.nodeTargets.forEach((node) => {
      ...
      const content = node.querySelector("[data-rb-tree-target='content']")
      const toggle = node.querySelector("[data-rb-tree-target='toggle']")
      if (content) {
  ...
  handleKeydown(event) {
    const node = event.target.closest("[data-rb-tree-target='node']")
    if (!node) return
