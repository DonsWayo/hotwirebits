  ...
  class PasswordInputComponent < Base
    ...
    def input_element
      tag.input(
        ...
        required: @required,
        class: merge_classes(
          'flex w-full rounded-md border border-hw-input bg-hw-background text-hw-foreground ring-offset-hw-background',
    ...
    def toggle_button
      tag.button(
        type: 'button',
        class: 'absolute right-0 top-0 flex h-full items-center px-3 text-hw-muted-foreground hover:text-hw-foreground',
        data: { action: 'click->hw-password#toggle' }
      ) do
    ...
    def strength_meter
      tag.div(class: 'mt-2', data: { rb_password_target: 'meter' }) do
        tag.div(class: 'flex gap-1 mb-1') do
          4.times.map do |_i|
            tag.div(class: 'h-1 w-full rounded-full bg-hw-muted', data: { rb_password_target: 'bar' })
          end.join.html_safe
        end +
          tag.p(class: 'text-xs text-hw-muted-foreground', data: { rb_password_target: 'label' },
