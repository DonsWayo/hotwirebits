...
module RailsBits
  class RatingComponent < Base
    ...
    def call
      tag.div(class: 'flex items-center gap-0.5', data: { controller: 'rb-rating' }) do
        safe_join([
                    hidden_input,
                    *@max.times.map do |i|
                      star_button(i + 1)
    ...
    def star_button(index)
      ...
      tag.button(
        type: 'button',
        class: merge_classes('text-rb-muted-foreground', filled ? 'text-yellow-400' : ''),
        data: { action: @readonly ? nil : 'click->rb-rating#select', value: index },
        disabled: @readonly
      ) do
        if @icon == :star
