...
module RailsBits
  class ChipComponent < Base
    ...
    def chip_classes
      ...
      merge_classes(
        'inline-flex items-center rounded-full border border-rb-border bg-rb-background',
        'text-rb-foreground font-medium',
        sizes[@size] || sizes[:md]
      )
    end
    ...
    def dismiss_button
      tag.button(type: 'button',
                 class: 'ml-0.5 -mr-1 h-4 w-4 rounded-full hover:bg-rb-muted p-0.5 focus:outline-none') do
