  ...
  class SpinnerComponent < Base
    ...
    def call
      tag.div(class: 'inline-flex items-center gap-2', role: 'status', **@extra_attrs) do
        safe_join([
          tag.svg(class: merge_classes('animate-spin text-rb-muted-foreground', SIZES[@size] || SIZES[:md]),
          ...
          (@show_label ? tag.span(@label, class: 'text-sm text-rb-muted-foreground') : nil),
