...
module RailsBits
  class StepsComponent < Base
    ...
    def step_circle_classes(state)
      ...
      case state
      when :completed then merge_classes(base, 'bg-rb-primary text-rb-primary-foreground')
      when :current then merge_classes(base, 'border-2 border-rb-primary text-rb-primary')
      else merge_classes(base, 'border-2 border-rb-border text-rb-muted-foreground')
      end
    end
    ...
    def connector_classes(idx)
      ...
      merge_classes(base, completed ? 'bg-rb-primary' : 'bg-rb-border')
    end
  end
end
