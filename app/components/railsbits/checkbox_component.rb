  ...
  class CheckboxComponent < Base
    ...
    def input_element
      attrs = {
        ...
        disabled: @disabled,
        class: input_classes,
        data: { action: 'change->rb-checkbox#toggle' }
      }
    ...
    def check_icon
      tag.span(class: 'pointer-events-none absolute inset-0 flex items-center justify-center') do
        tag.svg(class: merge_classes('text-rb-primary-foreground', SIZES[@size] || SIZES[:md]),
    ...
    def input_classes
      error_classes = @error ? 'border-rb-destructive' : 'border-rb-input'
      merge_classes(
        'peer shrink-0 rounded-sm border bg-rb-background ring-offset-rb-background',
        'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rb-ring focus-visible:ring-offset-2',
        'disabled:cursor-not-allowed disabled:opacity-50',
        'checked:bg-rb-primary checked:border-rb-primary',
        error_classes,
        SIZES[@size] || SIZES[:md]
      )
    end
